...
public class DEGenRecipeTypeRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<String> strings;

	public DEGenRecipeTypeRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<DEGenRecipeTypeRecipe> {
		private Type() {
		}

		public static final RecipeType<DEGenRecipeTypeRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<DEGenRecipeTypeRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<DEGenRecipeTypeRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.STRING.listOf().fieldOf("strings").forGetter(recipe -> recipe.strings)).apply(builder, DEGenRecipeTypeRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, DEGenRecipeTypeRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<DEGenRecipeTypeRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, DEGenRecipeTypeRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static DEGenRecipeTypeRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new DEGenRecipeTypeRecipe(outputs, inputs, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, DEGenRecipeTypeRecipe recipe) {
