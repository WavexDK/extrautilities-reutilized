...
public class GoldenLassoRTRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<String> strings;

	public GoldenLassoRTRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<GoldenLassoRTRecipe> {
		private Type() {
		}

		public static final RecipeType<GoldenLassoRTRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<GoldenLassoRTRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<GoldenLassoRTRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.STRING.listOf().fieldOf("strings").forGetter(recipe -> recipe.strings)).apply(builder, GoldenLassoRTRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, GoldenLassoRTRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<GoldenLassoRTRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, GoldenLassoRTRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static GoldenLassoRTRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new GoldenLassoRTRecipe(outputs, inputs, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, GoldenLassoRTRecipe recipe) {
