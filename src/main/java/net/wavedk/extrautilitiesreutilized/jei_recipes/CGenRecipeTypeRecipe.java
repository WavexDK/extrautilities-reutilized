...
public class CGenRecipeTypeRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<String> strings;

	public CGenRecipeTypeRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<CGenRecipeTypeRecipe> {
		private Type() {
		}

		public static final RecipeType<CGenRecipeTypeRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<CGenRecipeTypeRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<CGenRecipeTypeRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.STRING.listOf().fieldOf("strings").forGetter(recipe -> recipe.strings)).apply(builder, CGenRecipeTypeRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, CGenRecipeTypeRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<CGenRecipeTypeRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, CGenRecipeTypeRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static CGenRecipeTypeRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new CGenRecipeTypeRecipe(outputs, inputs, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, CGenRecipeTypeRecipe recipe) {
