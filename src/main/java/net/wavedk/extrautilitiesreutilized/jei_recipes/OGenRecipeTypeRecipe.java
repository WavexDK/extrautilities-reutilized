...
public class OGenRecipeTypeRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<String> strings;

	public OGenRecipeTypeRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<OGenRecipeTypeRecipe> {
		private Type() {
		}

		public static final RecipeType<OGenRecipeTypeRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<OGenRecipeTypeRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<OGenRecipeTypeRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.STRING.listOf().fieldOf("strings").forGetter(recipe -> recipe.strings)).apply(builder, OGenRecipeTypeRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, OGenRecipeTypeRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<OGenRecipeTypeRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, OGenRecipeTypeRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static OGenRecipeTypeRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new OGenRecipeTypeRecipe(outputs, inputs, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, OGenRecipeTypeRecipe recipe) {
