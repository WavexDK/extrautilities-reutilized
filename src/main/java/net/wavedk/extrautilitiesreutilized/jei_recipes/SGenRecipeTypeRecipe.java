...
public class SGenRecipeTypeRecipe implements Recipe<RecipeInput> {
	private final List<ItemStack> output;
	private final NonNullList<Ingredient> recipeItems;
	private final List<String> strings;

	public SGenRecipeTypeRecipe(List<ItemStack> output, NonNullList<Ingredient> recipeItems, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<SGenRecipeTypeRecipe> {
		private Type() {
		}

		public static final RecipeType<SGenRecipeTypeRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<SGenRecipeTypeRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<SGenRecipeTypeRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.STRING.listOf().fieldOf("strings").forGetter(recipe -> recipe.strings)).apply(builder, SGenRecipeTypeRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, SGenRecipeTypeRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<SGenRecipeTypeRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, SGenRecipeTypeRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static SGenRecipeTypeRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new SGenRecipeTypeRecipe(outputs, inputs, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, SGenRecipeTypeRecipe recipe) {
