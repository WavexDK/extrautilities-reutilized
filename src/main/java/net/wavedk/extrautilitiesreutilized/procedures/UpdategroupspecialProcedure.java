...
public class UpdategroupspecialProcedure {
	public static void execute(Entity entity) {
		...
		if (entity.getData(EuruModVariables.PLAYER_VARIABLES).group_count_special > 0) {
			fobj = EuruModVariables.unified_config.get("gp_efficiency_manager").getAsJsonObject();
			obj2 = fobj.get("special").getAsJsonObject();
			{
				EuruModVariables.PlayerVariables _vars = entity.getData(EuruModVariables.PLAYER_VARIABLES);
				_vars.group_efficiency_special = obj2.get("efficiency").getAsDouble();
				_vars.group_cutoff_special = obj2.get("efficiency_cutoff").getAsDouble();
				_vars.markSyncDirty();
			}
			raw = entity.getData(EuruModVariables.PLAYER_VARIABLES).group_raw_special;
			count = entity.getData(EuruModVariables.PLAYER_VARIABLES).group_count_special;
			cutoff = obj2.get("efficiency_cutoff").getAsDouble();
			eff = obj2.get("efficiency").getAsDouble();
			if (count > cutoff) {
				{
					EuruModVariables.PlayerVariables _vars = entity.getData(EuruModVariables.PLAYER_VARIABLES);
					_vars.group_update_special = Math.floor((raw - (raw / count) * (count - cutoff) + Math.pow((raw / count) * (count - cutoff), eff)) * 100) / 100;
				...
				{
					EuruModVariables.PlayerVariables _vars = entity.getData(EuruModVariables.PLAYER_VARIABLES);
					_vars.group_update_special = Math.floor(raw * 100) / 100;
