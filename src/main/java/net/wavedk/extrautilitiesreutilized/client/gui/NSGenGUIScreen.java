...
import net.wavedk.extrautilitiesreutilized.network.NSGenGUIButtonMessage;
...
public class NSGenGUIScreen extends AbstractContainerScreen<NSGenGUIMenu> implements EuruModScreens.ScreenAccessor {
	...
	private static final ResourceLocation BACKGROUND = ResourceLocation.parse("euru:textures/screens/ns_gen_gui.png");
	...
	public void init() {
		super.init();
		button_empty = new PlainTextButton(this.leftPos + 89, this.topPos + 31, 24, 20, Component.translatable("gui.euru.ns_gen_gui.button_empty"), e -> {
			int x = NSGenGUIScreen.this.x;
			int y = NSGenGUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new NSGenGUIButtonMessage(0, x, y, z));
				NSGenGUIButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}, this.font);
		this.addRenderableWidget(button_empty);
		button_empty1 = new PlainTextButton(this.leftPos + 89, this.topPos + 35, 24, 20, Component.translatable("gui.euru.ns_gen_gui.button_empty1"), e -> {
			int x = NSGenGUIScreen.this.x;
			int y = NSGenGUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new NSGenGUIButtonMessage(1, x, y, z));
				NSGenGUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}, this.font);
		...
		imagebutton_blank_x20 = new ImageButton(this.leftPos + 4, this.topPos + 16, 20, 20, new WidgetSprites(ResourceLocation.parse("euru:textures/screens/blank_x20.png"), ResourceLocation.parse("euru:textures/screens/blank_whiteborder_x20.png")),
				e -> {
					...
					int y = NSGenGUIScreen.this.y;
					if (true) {
						PacketDistributor.sendToServer(new NSGenGUIButtonMessage(2, x, y, z));
						NSGenGUIButtonMessage.handleButtonAction(entity, 2, x, y, z);
					}
				}) {
			@Override
