...
import net.wavedk.extrautilitiesreutilized.world.inventory.OGenGUIMenu;
import net.wavedk.extrautilitiesreutilized.procedures.*;
import net.wavedk.extrautilitiesreutilized.network.OGenGUIButtonMessage;
...
public class OGenGUIScreen extends AbstractContainerScreen<OGenGUIMenu> implements EuruModScreens.ScreenAccessor {
	...
	private static final ResourceLocation BACKGROUND = ResourceLocation.parse("euru:textures/screens/o_gen_gui.png");
	...
	private static final ResourceLocation IMAGE_29 = ResourceLocation.parse("euru:textures/screens/sgenbackdrop2-off.png");
	private static final ResourceLocation IMAGE_30 = ResourceLocation.parse("euru:textures/screens/sgenbackdrop2-on.png");
	...
	public OGenGUIScreen(OGenGUIMenu container, Inventory inventory, Component text) {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (SUTooltipGenProcedure.execute(world, x, y, z))
			if (mouseX > leftPos + 2 && mouseX < leftPos + 26 && mouseY > topPos + 38 && mouseY < topPos + 62) {
				guiGraphics.renderTooltip(font, Component.translatable("gui.euru.o_gen_gui.tooltip_speed_upgrade"), mouseX, mouseY);
				customTooltipShown = true;
			}
	...
	public void init() {
		super.init();
		button_empty = new PlainTextButton(this.leftPos + 89, this.topPos + 31, 24, 20, Component.translatable("gui.euru.o_gen_gui.button_empty"), e -> {
			int x = OGenGUIScreen.this.x;
			int y = OGenGUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new OGenGUIButtonMessage(0, x, y, z));
				OGenGUIButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}, this.font);
		this.addRenderableWidget(button_empty);
		button_empty1 = new PlainTextButton(this.leftPos + 89, this.topPos + 35, 24, 20, Component.translatable("gui.euru.o_gen_gui.button_empty1"), e -> {
			int x = OGenGUIScreen.this.x;
			int y = OGenGUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new OGenGUIButtonMessage(1, x, y, z));
				OGenGUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}, this.font);
		...
		imagebutton_blank_x20 = new ImageButton(this.leftPos + 4, this.topPos + 16, 20, 20, new WidgetSprites(ResourceLocation.parse("euru:textures/screens/blank_x20.png"), ResourceLocation.parse("euru:textures/screens/blank_whiteborder_x20.png")),
				e -> {
					...
					int y = OGenGUIScreen.this.y;
					if (true) {
						PacketDistributor.sendToServer(new OGenGUIButtonMessage(2, x, y, z));
						OGenGUIButtonMessage.handleButtonAction(entity, 2, x, y, z);
					}
				}) {
			@Override
